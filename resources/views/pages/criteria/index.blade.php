<x-layout>
        ...
        <dialog id="edit_{{ $row->id }}" class="modal">
            <div class="modal-box bg-white rounded-lg shadow dark:bg-gray-700">
                <div class="relative p-4 w-full max-w-md max-h-full">
                    ...
                    <!-- Modal body -->
                    <form class="p-4 md:p-5" action="{{ url("/criteria/$row->id") }}" method="POST">
                        @csrf @method('patch')
                        <div class="grid gap-4 mb-4 grid-cols-2">
                            <div class="col-span-2">
                                <label for="name"
                                    class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Name</label>
                                <input type="text" name="criteria" id="name" value="{{ $row->name }}"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                                    placeholder="Type criteria name" required>
                            </div>
    ...
    @endforeach
    @foreach ($data as $row)
        <dialog id="delete_{{ $row->id }}" class="modal modal-bottom sm:modal-middle">
            <div class="">
                <div
                    class="relative bg-white rounded-lg shadow dark:bg-gray-700 modal-box p-4 w-full max-w-md max-h-full">
                    <form method="dialog">
                    ...
                    <div class="p-4 md:p-5 text-center">
                        ...
                        <h3 class="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">Are you sure you want
                            to delete this criteria?</h3>
                        <form action="{{ url("/criteria/$row->id") }}" method="POST">
                            @csrf @method('delete')
                            <form method="dialog">
